...
namespace Test3
{
	...
	public partial class FormQuestions : Form
	{
		...
		public FormQuestions()
		{
			...
			Anketa_1DataSetTableAdapters.ПерсоналTableAdapter cv = new Anketa_1DataSetTableAdapters.ПерсоналTableAdapter();
			
			var cg = cv.GetDataBy2Search(FormMain.idPerson);

			
			int temp = 0;
			foreach (ПерсоналRow c1 in cg.Rows)
			{
				temp = c1.КодПодразделения;
				break;
			}
			Anketa_1DataSetTableAdapters.ПодразделенияTableAdapter gt = new Anketa_1DataSetTableAdapters.ПодразделенияTableAdapter();
			var kj = gt.GetDataBy1(temp);

			foreach (ПодразделенияRow c1 in kj.Rows)
			{
				temp = c1.КодВида;
				break;
			}
			...
			if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
			{
				...
				else
				{
					MessageBox.Show("Ваш результат: "
							+ FormMain.countRightResult + " верных ответов из 5", FormMain.fio);
					FormMain.countRightResult = 0;
					// добавить результат и ид человека в базу данных. В ноыую таблицу.
					//Anketa_1DataSetTableAdapters.ПерсоналTableAdapter pTA = new Anketa_1DataSetTableAdapters.ПерсоналTableAdapter();
					//pTA.Update(FormMain.idPerson);
					Close();
				}
				if (!repeat)
				{
